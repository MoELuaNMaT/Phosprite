...
func test_default_thresholds_are_centralized_and_copyable() -> void:
	...
	check_almost_eq(config.tap_slop_px, 12.0, 0.00001, "tap slop must use the D0 provisional default")
	check_almost_eq(
		config.drag_threshold_px, 12.0, 0.00001, "drag acquisition must use the D0 provisional default"
	)
	...
	check_almost_eq(config.double_tap_distance_px, 20.0, 0.00001, "double tap distance must be centralized")

	var copy := config.duplicate_config()
	copy.tap_slop_px = 7.0
	check_almost_eq(config.tap_slop_px, 12.0, 0.00001, "runtime tuning copies must not mutate shared defaults")
...
func test_single_tap_waits_for_double_tap_window_before_commit() -> void:
	...
	check_eq(recognizer.begin(1, Vector2(20, 30), 0).size(), 0, "touch begin must not commit a gesture")
	check_eq(recognizer.release(1, Vector2(25, 32), 100).size(), 0, "first tap must wait for double-tap disambiguation")
	check_true(recognizer.has_pending_tap(), "first tap must remain pending inside the double-tap window")
	check_eq(recognizer.advance_time(400).size(), 0, "tap stays pending through the configured interval boundary")
...
func test_drag_acquisition_cancels_tap_and_has_explicit_lifecycle() -> void:
	...
	check_eq(recognizer.move(3, Vector2(6, 2), 10).size(), 0, "sub-threshold movement must remain undecided")
	...
	check_eq(start_events[0]["gesture"], RECOGNIZER.Gesture.DRAG_START, "threshold crossing must emit DRAG_START")
	...
	check_eq(update_events[0]["gesture"], RECOGNIZER.Gesture.DRAG_UPDATE, "active drag must emit DRAG_UPDATE")
	var end_events := recognizer.release(3, Vector2(20, 5), 40)
	check_eq(end_events.size(), 1, "drag release must emit one end event")
	check_eq(end_events[0]["gesture"], RECOGNIZER.Gesture.DRAG_END, "drag release must emit DRAG_END")
...
func test_motion_past_tap_slop_without_drag_is_not_reinterpreted_as_tap() -> void:
	...
	check_eq(recognizer.move(4, Vector2(15, 0), 100).size(), 0, "mid-band movement must not fabricate a drag")
	check_eq(recognizer.advance_time(500).size(), 0, "movement past tap slop must invalidate long press")
	check_eq(recognizer.release(4, Vector2(15, 0), 550).size(), 0, "invalidated contact must not become a tap")
...
func test_long_press_fires_once_and_release_does_not_also_tap() -> void:
	...
	check_eq(recognizer.advance_time(449).size(), 0, "long press must not fire before its threshold")
	...
	check_eq(recognizer.release(5, Vector2(40, 40), 800).size(), 0, "releasing long press must not emit a tap")
	check_true(not recognizer.has_pending_tap(), "long press release must not create a tap candidate")
...
func test_fast_nearby_taps_commit_one_double_tap_without_single_taps() -> void:
	...
	check_eq(events[0]["gesture"], RECOGNIZER.Gesture.DOUBLE_TAP, "qualifying pair must emit DOUBLE_TAP")
	check_true(not recognizer.has_pending_tap(), "double tap must consume the pending first tap")
	check_eq(recognizer.advance_time(1000).size(), 0, "double tap must not leak a delayed single tap")
...
func test_cancel_all_clears_active_contacts_and_delayed_actions() -> void:
	...
	check_eq(events[0]["gesture"], RECOGNIZER.Gesture.CANCEL, "first active contact must be cancelled")
	check_eq(events[1]["gesture"], RECOGNIZER.Gesture.CANCEL, "second active contact must be cancelled")
...
func test_pointer_ownership_is_exclusive_and_releasable_by_owner() -> void:
	...
	check_true(not ownership.acquire(1, &"Canvas"), "a competing owner must not steal an acquired pointer")
	check_eq(ownership.owner_of(1), &"LayerPanel", "registry must report the authoritative owner")
	check_true(not ownership.release(1, &"Canvas"), "wrong owner must not release another surface's pointer")
	check_true(ownership.release(1, &"LayerPanel"), "authoritative owner must be able to release its pointer")
	...
	check_eq(ownership.cancel_owner(&"Palette"), PackedInt32Array([2, 4]), "owner teardown must release all of its pointers deterministically")
	...
	check_eq(snapshot.get(3, &""), &"Canvas", "snapshot must preserve the previous owner for cancellation routing")
