...
`WorkspaceBuiltinModules` assigns stable module IDs to the current editor panels, including Tools, contextual tool options, Canvas Preview, Palette, Color Picker, Timeline, Tiles, and the 3D Object Tree. Global Tool Options is no longer a standalone Workspace module: its existing controls are mounted into the right side of Animation Timeline's Workspace header, before the Float/Collapse actions. The controls remain visible when the Timeline body is collapsed and are not part of the Timeline content area.
