...
| ID | 必测 | 操作 | 预期 |
|---|---|---|---|
| A01 | 是 | 对最终 IPA 完成签名/侧载并首次启动 | App 可正常启动，无崩溃、黑屏、永久 loading |
| A02 | 是 | 清除应用数据后冷启动 | 直接进入 Project Gallery，不进入 Editor |
| A03 | 是 | 冷启动观察启动过程 | 不出现旧 Splash |
| A04 | 是 | 冷启动且没有任何 recovery | 不出现全局“上次异常退出/恢复”弹窗 |
| A05 | 是 | 杀掉 App 后再次普通启动 | 仍先进入 Gallery，不自动打开上次项目 |
| A06 | 是 | Gallery 横屏启动 | 顶栏、项目区、Import/New 均在安全区内，无裁切 |
| A07 | 是 | Gallery 竖屏启动 | 同上，无控件越界 |
| A08 | 是 | Gallery 前后台切换 3 次 | 不闪退、不重复初始化、不出现重复弹窗 |
