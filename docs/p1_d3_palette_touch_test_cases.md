...
11. Long-press a non-empty swatch for about 450 ms, then drag it onto another swatch. Once reorder ownership is acquired, a visible copy of the source swatch must follow the finger so the gesture has continuous feedback. The swatch that remains in the original slot must gain a distinct inset dashed outline while the drag is active; any existing Primary/Secondary selection highlight must remain unchanged. The following preview itself must not show this dashed source marker. The existing Palette reorder transaction must run on release. Undo/Redo must restore/reapply the reorder.
...
18. Drag outside the visible Palette area after acquiring reorder and release outside. It must not perform an accidental swap, and both the transient drag preview and source-slot dashed outline must disappear.
