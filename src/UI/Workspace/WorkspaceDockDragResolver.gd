...
static func resolve(
	...
	var zone := _find_zone(pointer, zone_rects, layout)
	...
	if layout.get_module_zone(module_id) == WorkspaceDockLayout.DockZone.NONE:
		var definition := layout._manager.get_definition(module_id)
		if definition != null:
			module_size = definition.get_constrained_preferred_size()
...
static func _find_zone(pointer: Vector2, zone_rects: Dictionary, layout: WorkspaceDockLayout) -> int:
...
static func _find_insertion_index(pointer: Vector2, zone: int, entries: Array) -> int:
	var horizontal := zone == WorkspaceDockLayout.DockZone.TOP or zone == WorkspaceDockLayout.DockZone.BOTTOM
